...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    new_para = [s for s in paragraphs if select(s)]
    if k+1<=len(new_para):
        return new_para[k]
    else:
        return ''
...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def judge(para):
        para_split = split(remove_punctuation(lower(para)))
        for i in para_split:
            for j in topic:
                if i==j:
                    return True
    ...
    if len(typed_words)==0 and len(source_words)==0:
        ...
        for i in range(d):
            if source_words[i] == typed_words[i]:
                s = s + 1
        return s/len(typed_words)*100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in word_list:
        if typed_word==i:
            return typed_word
    dif_list = [diff_function(typed_word,i,limit) for i in word_list]
    dif_min_id = 0
    for i in range(len(dif_list)):
        if dif_list[dif_min_id]> dif_list[i]:
            dif_min_id = i
    if dif_list[dif_min_id]>limit:
        return typed_word
    else:
        return word_list[dif_min_id]
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    len_dif = abs(len(source)-len(typed))
    if len_dif>limit:
        return limit+1
    elif len_dif>0:
        if len(source)>len(typed):
            return feline_fixes(typed,source[0:len(typed)],limit)+len_dif
        else:
            return feline_fixes(typed[0:len(source)],source,limit)+len_dif
    else:
        if len(typed)==0 and len(source)==0:
            return 0
        else:
            return feline_fixes(typed[1:],source[1:],limit-(typed[0]!=source[0]))+(typed[0]!=source[0])
    # END PROBLEM 6
