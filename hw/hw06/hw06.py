...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        more_year = Mint.present_year-self.year
        return self.cents+(more_year>50)*(more_year-50)
...
class VendingMachine:
    ...
    def __init__(self,item,price):
        self.items = {'item':item,'price':price,'number':0,'balance':0}
    def vend(self):
        if self.items['number']==0:
            return 'Nothing left to vend. Please restock.'
        else:
            diff = self.items['price'] - self.items['balance']
            item_name = self.items['item']
            if diff>0:               
                return f'Please add ${diff} more funds.'
            elif diff==0:               
                self.items['balance'] = 0 
                self.items['number'] -= 1
                return f'Here is your {item_name}.'
            else:
                self.items['balance'] = 0 
                self.items['number'] -= 1
                return f'Here is your {item_name} and ${-diff} change.'
    def restock(self,number):
        self.items['number'] += number
        item = self.items['item']
        number_now = self.items['number']
        return f'Current {item} stock: {number_now}'
    def add_funds(self,money):
        if self.items['number']==0:
            return f'Nothing left to vend. Please restock. Here is your ${money}.'
        else:
            self.items['balance'] += money
            balance_now = self.items['balance']
            return f'Current balance: ${balance_now}'
...
class VirFib():
    ...
    def next(self):
        "*** YOUR CODE HERE ***"
        new_p = VirFib(self.value,self.next_value)
        a = new_p.value + new_p.next_value
        return VirFib(new_p.next_value,a)
