...
def count_coins(change):
    ...
    """
    "*** YOUR CODE HERE ***"
    def partition_max(j,k):
        if j<0:
            return 0
        elif j==0 or j==1:
            return 1
        elif j<k:
            return partition_max(j,next_smaller_coin(k))
        else:
            if next_smaller_coin(k):
                return partition_max(j,next_smaller_coin(k))+partition_max(j-k,k)
            else:
                return 1
    return partition_max(change,25)
