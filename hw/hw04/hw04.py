...
def balanced(m):
    ...
    if is_planet(m):
        ...
        if  (left_t == right_t) and balanced(end(left(m))) and balanced(end(right(m))):
            return True
        else:
            return False
...
def has_path(t, word):
    ...
    if len(word)==1 and word==label(t):
        ...
        find_branches = [has_path(b,word[1:]) for b in branches(t)]
        return any(find_branches)      
    else:
        return False
...
def sub_interval(x, y):
    ...
    y_low = lower_bound(y)
    y_up  = upper_bound(y)
    return interval(x_low-y_up,x_up-y_low)
