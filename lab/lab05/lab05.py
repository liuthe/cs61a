def flatten(s):
    ...
    """
    "*** YOUR CODE HERE ***"
    if all([type(s[_])!=list for _ in range(len(s))]):
        return s
    else:
        result = []
        for k in range(len(s)):
            if type(s[k])==list:
                result = result + flatten(s[k])
            else:
                result.append(s[k])
        return result
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    mycity = make_city('my_city', lat, lon)
    dis1 = distance(mycity,city_a)
    dis2 = distance(mycity,city_b)
    if dis1<=dis2:
